...
import { LoanNFT, GuruNFT, ParticipantNFT, ReputationNFT, BarterNFT } from "../../abcdefi-token/typechain-types";
...
describe("ABCDeFi NFT Smart Contract Suite", function () {
  ...
  beforeEach(async function () {
    [owner, admin, user1, user2] = await hardhatEthers.getSigners();

    const LoanFactory = await hardhatEthers.getContractFactory("LoanNFT");
    loanNFT = await LoanFactory.deploy(admin.address);

    const GuruFactory = await hardhatEthers.getContractFactory("GuruNFT");
    ...
    const BarterFactory = await hardhatEthers.getContractFactory("BarterNFT");
  ...
  describe("1. LoanNFT", function () {
    it("should mint loan certificate NFT and return details", async function () {
      ...
      expect(details.loanAmount).to.equal(ethers.parseUnits("5000", 18));
    });
  });
