...
describe("CollateralVault Contract Suite", function () {
  ...
  beforeEach(async function () {
    [owner, admin, operator, borrower, liquidator] = await ethers.getSigners();
    ...
    vault = (await VaultFactory.deploy(admin.address)) as unknown as CollateralVault;
  ...
  describe("2. Post-Repayment Collateral Release & Liquidation", function () {
    ...
    it("should allow VAULT_OPERATOR_ROLE to release collateral post-repayment", async function () {
      ...
      const initialBal = await ethers.provider.getBalance(borrower.address);
      ...
      const finalBal = await ethers.provider.getBalance(borrower.address);
