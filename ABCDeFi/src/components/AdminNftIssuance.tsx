  ...
  const submit = async () => {
    ...
    try {
      ...
      const result = form.nftType === 'Franchise'
        ? await mintFranchise({ franchisee: form.recipient, franchiseName: form.name, territoryCode: form.territoryCode, territoryName: form.territory, level: Number(form.tier), legionNFTId: form.legionNFTId, priceUSD: form.priceUSD, commissionBps: form.commissionBps, tokenURI: storedAsset.metadataUri, ipfsCID: storedAsset.ipfsCid || '' }, onSubmitted)
        : await mintLegion({ recipient: form.recipient, name: form.name, territory: form.territory, level: Number(form.tier), parentId: form.parentId, character: form.character, metadataURI: storedAsset.metadataUri, population: form.population, treasuryShareBps: form.treasuryShareBps }, onSubmitted);
