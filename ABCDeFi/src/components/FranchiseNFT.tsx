...
export const FranchiseNFT: React.FC = () => {
  ...
  const submitListing = async (franchise: FranchiseRecord) => {
    if (transactionState === 'awaiting-wallet' || transactionState === 'confirming') return;
    if (!address || !isConnected) { setTransactionState('error'); setTransactionMessage('Connect the Franchise owner wallet before listing.'); return; }
    if (!isCorrectNetwork) { setTransactionState('error'); setTransactionMessage('Switch to Hardhat Local (31337) before listing.'); return; }
    if (franchise.transferLocked) { setTransactionState('error'); setTransactionMessage('This Franchise NFT is currently under the protocol transfer lock.'); return; }
    setTransactionState('awaiting-wallet'); setTransactionHash(null); setTransactionMessage('Confirm the Franchise marketplace listing in MetaMask.');
    try {
      const result = await listFranchiseOnMarketplace(franchise.tokenId, listingPrices[franchise.tokenId] || '', (hash, stage) => {
        setTransactionHash(hash); setTransactionState('confirming'); setTransactionMessage(`${stage} submitted. Waiting for confirmation…`);
      });
      await refresh(); setTransactionState('success'); setTransactionMessage(`Franchise certificate listed on-chain as listing #${result.listingId || 'confirmed'}.`);
    } catch (reason) { setTransactionState('error'); setTransactionMessage(franchiseErrorMessage(reason)); }
  };
    ...
    <header className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-xs text-slate-400">Ownership, territory details, and the three-year transfer lock are read from the canonical FranchiseNFT contract. This deployment has no public purchase, rebate, or commission payout. A certificate can only be listed through the canonical marketplace after its on-chain transfer lock has expired.</p>
        </div>
...
const FranchiseCard = ({ franchise, listingPrice, onListingPriceChange, onList, listingBusy }: { franchise: FranchiseRecord; listingPrice: string; onListingPriceChange: (value: string) => void; onList: () => void; listingBusy: boolean }) => <article className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-xs text-slate-200"><div className="flex justify-between gap-3"><div><p className="font-bold text-white">#{franchise.tokenId} · {franchise.franchiseName}</p><p className="mt-1 text-slate-400">{franchise.level} · {franchise.territoryName}</p></div><span className="rounded-full border border-slate-700 px-2 py-1 text-slate-300">{franchise.status}</span></div><dl className="mt-4 grid gap-x-5 gap-y-2 sm:grid-cols-2"><Field label="Territory code" value={franchise.territoryCode} /><Field label="Current owner" value={franchise.owner} /><Field label="Transfer lock" value={franchise.transferLocked ? formatRemaining(franchise.remainingLockSeconds) : 'Unlocked'} /><Field label="Lock expires" value={formatTimestamp(franchise.lockExpiryTimestamp)} /><Field label="Issued" value={formatTimestamp(franchise.purchaseTimestamp)} /><Field label="Recorded price" value={`${franchise.priceUSD} USD`} /><Field label="Recorded commission" value={`${franchise.commissionBps} bps`} /><Field label="Linked Legion ID" value={franchise.legionNFTId} /><Field label="Metadata URI" value={franchise.tokenUri || 'Unavailable'} /><Field label="IPFS CID" value={franchise.ipfsCID || 'Unavailable'} /><Field label="Mint transaction" value={franchise.mintTransactionHash || 'Not available'} /><Field label="Mint block" value={franchise.mintBlockNumber || 'Not available'} /></dl><MetadataPreview uri={franchise.tokenUri} /><div className="mt-4 border-t border-slate-800 pt-3"><p className="text-[11px] font-semibold text-slate-300">Marketplace</p>{franchise.transferLocked ? <p className="mt-1 text-[11px] text-amber-300">This Franchise NFT is currently under the protocol transfer lock.</p> : <div className="mt-2 flex gap-2"><input aria-label={`Listing price for Franchise #${franchise.tokenId}`} value={listingPrice} onChange={(event) => onListingPriceChange(event.target.value)} placeholder="ETH price" className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-900 px-2 py-1.5 text-xs text-white" /><button onClick={onList} disabled={listingBusy} className="rounded-lg bg-emerald-600 px-2 py-1.5 font-bold text-white disabled:opacity-50">List</button></div>}</div></article>;
