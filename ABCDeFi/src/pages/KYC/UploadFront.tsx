...
interface UploadFrontProps {
  docType: string;
  onContinue: () => void;
}

export const UploadFront: React.FC<UploadFrontProps> = ({ docType, onContinue }) => {
  const [uploaded, setUploaded] = useState<boolean>(false);

  return (
    <div className="space-y-6 text-slate-100 font-mono">
      <div>
        <h2 className="text-lg font-bold text-white tracking-tight">Step 3: Upload Front Side</h2>
        <p className="text-xs text-slate-400">Upload a clear scan of the front of your {docType}.</p>
      </div>
      ...
      {!uploaded ? (
        ...
        >
          ...
          <span className="font-bold text-indigo-300 text-xs">Choose File to Upload {docType} (Front)</span>
        ...
        <div className="bg-emerald-950/60 border border-emerald-500/40 p-5 rounded-2xl flex items-center justify-between text-xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-emerald-300">{docType} (Front) Uploaded</div>
              <div className="text-[10px] text-slate-400">OCR Readability: 100% Clear • 1.4 MB</div>
            </div>
          </div>
      ...
      >
        <span>Continue to Upload Back →</span>
