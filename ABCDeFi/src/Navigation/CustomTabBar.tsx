...
export const CustomTabBar = ({ state, navigation }: any) => {
 ...
 const getIcon = (routeName: string, isFocused: boolean) => {
  switch (routeName) {
    ...
    case "Finance":
      return isFocused ? ICO_white : ICO;   // reuse ICO icon for Finance

    case "Portfolio":
      return isFocused ? Home_white : Home; // reuse Home icon for Portfolio
        ...
        {state.routes.map((route: any, index: number) => {
          ...
          return (
            ...
            >
              ...
              >
                 {route.name === "AIScreen" ? "59C AI" : route.name === "Finance" ? "Finance" : route.name === "Portfolio" ? "Portfolio" : route.name}
              </Text>
