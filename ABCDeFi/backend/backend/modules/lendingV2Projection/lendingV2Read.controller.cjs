...
const requestJson = (request) => toJson({ borrower: request.borrower, principal: request.principal, collateral: request.collateral, term: request.term, state: request.state, lender: request.lender, metadataURI: request.metadataURI, metadataHash: request.metadataHash, loanId: request.loanId, initialLtvBps: request.initialLtvBps });
