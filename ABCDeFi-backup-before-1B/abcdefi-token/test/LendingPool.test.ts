...
describe("LendingPool Contract Suite", function () {
  ...
  beforeEach(async function () {
    [owner, admin, borrower] = await ethers.getSigners();
  ...
  describe("1. Collateral Deposits & Borrowing Limits", function () {
    ...
    it("should allow borrowing ABCD tokens up to 75% LTV", async function () {
      // Deposit 2 ETH collateral -> Max borrow value = 2 ETH * 1000 * 75% = 1,500 ABCD
      await lendingPool.connect(borrower).depositCollateral({ value: ethers.parseEther("2.0") });
    ...
    it("should revert borrowTokens if exceeding LTV threshold (e.g. 2000 ABCD on 2 ETH)", async function () {
      await lendingPool.connect(borrower).depositCollateral({ value: ethers.parseEther("2.0") });

      const excessiveAmount = ethers.parseUnits("2000", 18); // Max allowed is 1,500 ABCD
  ...
  describe("2. Repayment & Collateral Withdrawal", function () {
    ...
    it("should allow withdrawing collateral post-repayment", async function () {
      ...
      const initialEthBal = await ethers.provider.getBalance(borrower.address);
